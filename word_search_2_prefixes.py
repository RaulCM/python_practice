...
def search(i, j, board, words, prefixes, path, words_found):
    ...
    board[i][j] = '#'
    search(i + 1, j, board, words, prefixes, path, words_found)
    search(i - 1, j, board, words, prefixes, path, words_found)
    search(i, j + 1, board, words, prefixes, path, words_found)
    search(i, j - 1, board, words, prefixes, path, words_found)
    board[i][j] = c


def solve(board, words):
    prefixes = gen_prefixes(words)
    words_found = set()
    for i in range(len(board)):
        for j in range(len(board[0])):
            search(i, j, board, set(words), prefixes, "", words_found)
